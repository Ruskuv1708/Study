...
from modules.access_control.access_models import Workspace, User
from modules.access_control.access_enums import UserRole
from passlib.context import CryptContext
from core.config import settings
from modules.workspace_management.workspace_schemas import WorkspaceCreateSchema
...
class WorkspaceService:
    ...
    @staticmethod
    def activate_workspace(db: Session, workspace_id: UUID) -> dict:
        """Activate a workspace (reactivate users)"""
        workspace = db.query(Workspace).filter(Workspace.id == workspace_id).first()
        if not workspace:
            raise HTTPException(status_code=404, detail="Workspace not found")
        ...
        workspace.activated_at = datetime.now(timezone.utc)
        workspace.suspended_at = None

        db.commit()
        return {"message": f"Workspace '{workspace.name}' has been activated"}
